...
```{r}
CNDClm = crossvalidlm(data42[,2],data42[c(6)],5,100)
```
...
sd(CNDClm[,"WMAPE"])
sd(CNDClm[,"rmse"])
bbe = c(mean(CNDClm[,"arsq"]),mean(CNDClm[,"WMAPE"]),mean(CNDClm[,"rmse"]))
bbc = c(sd(CNDClm[,"arsq"]),sd(CNDClm[,"WMAPE"]),sd(CNDClm[,"rmse"]))
data.frame(bbe,bbc)
